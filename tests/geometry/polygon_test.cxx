...
float distance_to(geometry::plane pl, const geometry::clip_vertex& vertex)
{
    const auto& p = vertex.position;

    switch( pl ) {
        case geometry::plane::left:
            return p.w() + p.x();
        case geometry::plane::right:
            return p.w() - p.x();
        case geometry::plane::bottom:
            return p.w() + p.y();
        case geometry::plane::top:
            return p.w() - p.y();
        case geometry::plane::near:
            return p.w() + p.z();
        case geometry::plane::far:
            return p.w() - p.z();
    }
...
TEST_CASE("clip_against_plane returns an empty polygon for empty input")
{
    ...
    CHECK(geometry::clip_against_plane(input, geometry::plane::right).empty());
}
...
TEST_CASE("clip_against_plane preserves a polygon entirely inside")
{
    ...
    const auto output = geometry::clip_against_plane(input, geometry::plane::right);
...
TEST_CASE("clip_against_plane clips against every homogeneous clip plane")
{
    ...
    const std::array cases{
        plane_case{geometry::plane::left, {-3.0F, 0.0F, 0.0F, 2.0F}},
        plane_case{geometry::plane::right, {3.0F, 0.0F, 0.0F, 2.0F}},
        plane_case{geometry::plane::bottom, {0.0F, -3.0F, 0.0F, 2.0F}},
        plane_case{geometry::plane::top, {0.0F, 3.0F, 0.0F, 2.0F}},
        plane_case{geometry::plane::near, {0.0F, 0.0F, -3.0F, 2.0F}},
        plane_case{geometry::plane::far, {0.0F, 0.0F, 3.0F, 2.0F}},
    };
